...
function run_all {
    ...
    sbatch --partition=icb_rstrct --qos=icb_rstrct --ntasks=1 --cpus-per-task=${2} --mem-per-cpu=${3} --time=1-00:00:00 --job-name=met --output=${NODE_SBATCH_LOG} --error=${NODE_SBATCH_ERR} ${1}/slave_metrics.sh ${4} ${5} ${6} ${7} ${8} ${9} ${10} ${11}
    echo sbatch --partition=icb_rstrct --qos=icb_rstrct --ntasks=1 --cpus-per-task=${2} --mem-per-cpu=${3} --time=1-00:00:00 --job-name=met --output=${NODE_SBATCH_LOG} --error=${NODE_SBATCH_ERR} ${1}/slave_metrics.sh ${4} ${5} ${6} ${7} ${8} ${9} ${10} ${11}
    echo " "
...
for PARAM in scanorama_full mnn_full scanorama_embed
do
    for HVGS in 0 2000
    do
        METHOD=${PARAM%_*}
        ...
        INPUTFILE=/storage/groups/ml01/workspace/group.daniela/lung/Lung_atlas_final_fixed.h5ad
        INTEGRATIONFILE=/storage/groups/ml01/workspace/group.daniela/lung/output/Lung_atlas_final_fixed_${METHOD}_hvg${HVGS}.h5ad
