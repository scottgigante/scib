...
########
# Check cell label
method_list = ['scanorama','mnn','seurat','harmony','trvae','bbknn']
hvg = ['0','2000']
for fn in list_inputfile:
    print("\nLoading :"+fn)
    bname = os.path.basename(fn).split('.')[0]
    dname = os.path.dirname(fn).split('.')[0]
    
    b = anndata.read(fn)
    
    for m in method_list:
        for h in hvg:
            resfile = dname+"/output/"+bname+"_"+m+"_hvg"+h+".h5ad"
            if os.path.exists(resfile):
                print("\nLoading :"+resfile)
                a = anndata.read(dname+"/output/"+bname+"_"+m+"_hvg"+h+".h5ad")
                print("Diff between input.X and output.X is "+str(b.X.size - a.X.size))
